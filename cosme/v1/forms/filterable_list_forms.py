...
from cosme.v1.util import ERROR_MESSAGES, ref
...
class FilterableListForm(forms.Form):
    ...
    title = forms.CharField(
        max_length=250, required=False, widget=widgets.TextInput(attrs=title_attrs)
    )
    ...
    def get_page_set(self):
        ...
        return (
            self.filterable_pages.filter(query).distinct().order_by("-date_published")
        )
    ...
    def set_topics(self, page_ids):
        tags = Tag.objects.filter(
            v1_cosmetaggedpages_items__content_object__id__in=page_ids
    ...
    def set_authors(self, page_ids):
        authors = Tag.objects.filter(
            v1_cosmeauthoredpages_items__content_object__id__in=page_ids
        ).values_list("slug", "name")
    ...
    def clean(self):
        ...
        if (from_date and to_date) and to_date < from_date:
            ...
            self.cleaned_data["to_date"], data["to_date"] = from_date, data["from_date"]
            self.cleaned_data["from_date"], data["from_date"] = to_date, data_to_date
            self.data = data
        ...
        for f in self.fields:
            if field.html_name == f:
                self.fields[f].widget.attrs.update(
                    {"id": attr_id, "class": "a-text-input a-text-input__full"}
                )
    ...
    def generate_query(self):
        ...
        if self.is_bound:
            for query, field_name in zip(
                self.get_query_strings(), self.declared_fields
            ):
                if self.cleaned_data.get(field_name):
                    final_query &= Q((query, self.cleaned_data.get(field_name)))
        return final_query
