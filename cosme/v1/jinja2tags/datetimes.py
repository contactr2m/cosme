...
from cosme.v1.util.util import extended_strftime
...
def _convert_date(date, tz):
    if date and isinstance(date, basestring):
        date = parser.parse(date, default=datetime.datetime.today().replace(day=1))
...
class DatetimesExtension(Extension):
    def __init__(self, environment):
        ...
        self.environment.filters.update(
            {"date": get_date_string, "localtime": template_localtime}
        )
