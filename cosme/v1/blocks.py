...
from cosme.v1.atomic_elements import atoms
...
class Link(blocks.StructBlock):
    link_text = blocks.CharBlock(required=True, label="Text")
    page_link = blocks.PageChooserBlock(
        required=False, help_text="Link to a page in Wagtail.", label="Page"
    )
    ...
    def clean(self, value):
        ...
        if not cleaned.get("page_link") and not cleaned.get("external_link"):
            raise ValidationError(
                ...
                params={
                    "page_link": ["Either page or external link is required."],
                    "external_link": ["Either page or external link is required."],
                },
            )
        return cleaned
...
class NavItem(blocks.StructBlock):
    ...
    nav_items = blocks.ListBlock(
        blocks.StructBlock([("link", Link())]), label="Child items (mobile only)"
    )
...
class NavFooter(blocks.StructBlock):
    ...
    content = blocks.RichTextBlock(required=False, features=["link"])
