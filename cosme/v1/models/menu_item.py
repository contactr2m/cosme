...
class MenuItem(models.Model):
    ...
    page_link = models.ForeignKey(
        ...
        related_name="+",
        help_text=(
            "Link to Wagtail overview page for this menu item "
    ...
    column_1 = StreamField(
        [("nav_group", v1_blocks.NavGroup(label="Menu items group"))], blank=True
    )
    ...
    nav_footer = StreamField(
        [("nav_footer", v1_blocks.NavFooter(label="Menu footer"))], blank=True
    )

    panels = [
        MultiFieldPanel(
            [FieldPanel("link_text"), PageChooserPanel("page_link")],
    ...
    def get_content(self, draft):
        ...
        self.featured_content = (
            self.nav_groups.pop()
            if len(self.nav_groups)
            and self.nav_groups[-1].block_type == "featured_content"
            else None
        )
    ...
    def get_active_block(blocks, draft):
        ...
        return next(
            (
                ...
                if block.value.get("draft", "") == draft
                or (draft and i == len(blocks) - 1)
            ),
            None,
        )
