...
from wagtail.core.blocks import ListBlock, StructBlock, DateTimeBlock, \
    ChoiceBlock
...
class HomePage(COSMEPage):
    header = StreamField([('info_unit', molecules.InfoUnit()),
                          ('half_width_link_blob',
                           molecules.HalfWidthLinkBlob()),
                          ], blank=True)
    ...
    latest_updates = \
        StreamField([('posts',
                      ListBlock(
                          StructBlock(
                              [('categories',
                                ChoiceBlock(choices=ref.limited_categories,
                                            required=False)),
    ...
    # General content tab
    content_panels = \
        COSMEPage.content_panels + [StreamFieldPanel('header'),
